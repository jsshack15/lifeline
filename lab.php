...
<section id="one" class="wrapper style1 align-center">
			  <div class="container">
				...
				<div class="box alt">
				<?php 
				$sql=mysql_query("select labname,logo,link,description,logo2,logo3,logo4,logo5 from lablist where id='$id'");
				$row=mysql_fetch_row($sql);
				$lname=$row[0];
				$link=$row[2];
				$sql10=mysql_query("select address,phone from labbranch where labname='$lname'");
				$output=mysql_fetch_row($sql10);
				
								echo'
				<div id="slider">
<figure>
            <img height="426" class="image fit"  src="data:image;base64,'.$row[1].' " alt="" >
            <img   height="426" class="image fit" src="data:image;base64,'.$row[4].' " alt="" >
            ...
            <img   height="426"  class="image fit" src="data:image;base64,'.$row[7].' " >
</figure>
</div>
	...
	<header>
			<?php   echo'<p><span style="font-size:25px;color:white;font-weight:bold;">'.$row[0].'</span></br><a id="showonmap"href="showmap.php?id='.$id.'"class="button small fit">show on map</a></p>';?>
			       </header>
